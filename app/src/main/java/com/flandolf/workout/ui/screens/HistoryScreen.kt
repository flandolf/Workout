                    ...
                    Column(modifier = Modifier
                        ...
                        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                            Column {
                                ...
                                Text("Duration: ${w.workout.durationSeconds / 60} min", style = MaterialTheme.typography.bodySmall)
                            }
                        ...
                        if (expanded) {
                            ...
                            Column(modifier = Modifier.fillMaxWidth()) {
                                for (ex in w.exercises) {
                                    Text(ex.exercise.name, style = MaterialTheme.typography.titleSmall, modifier = Modifier.padding(vertical = 4.dp))
                                    for ((i, s) in ex.sets.withIndex()) {
                                        Row(modifier = Modifier.fillMaxWidth().padding(start = 8.dp, top = 2.dp, bottom = 2.dp), horizontalArrangement = Arrangement.SpaceBetween) {
                                            Text("Set ${i + 1}")
